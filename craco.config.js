...
webpackConfig.devServer = (devServerConfig) => {
  // Patch: react-scripts uses webpack-dev-server v4 API; we have v5
  ...
  // v4 `https` -> v5 `server: { type: 'https', options }`
  if ('https' in devServerConfig) {
    const https = devServerConfig.https;
    delete devServerConfig.https;
    if (https) {
      devServerConfig.server = typeof https === 'object' ? { type: 'https', options: https } : 'https';
    }
  }
  // v4 `http2` removed in v5
  if ('http2' in devServerConfig) {
    delete devServerConfig.http2;
  }
  // Allow preview/proxy host
